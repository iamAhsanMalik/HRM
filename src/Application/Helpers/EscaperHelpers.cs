...
public class EscaperHelpers : IEscaperHelpers
{
    const string _invalidChars = @"""\/?:<>*|";
    const string _escapeChar = "%";
    public Regex GetEscaperChar()
    {
        return new Regex(
        "[" + Regex.Escape(_escapeChar + _invalidChars) + "]",
        RegexOptions.Compiled);
    }
    public Regex GetUnescaperChar()
    {
        return new Regex(
            Regex.Escape(_escapeChar) + "([0-9A-Z]{4})",
            RegexOptions.Compiled);
    }
    ...
    public string DecodePath(string token)
    {
    ...
    public string GetSecureUrl(string fullPath, string fileName)
    {
        ...
        string EncodedPath = EncodePath(fullPath);
        string secureUrl = "";
        if (!string.IsNullOrEmpty(EncodedPath) && File.Exists(fullPath))
        {
            secureUrl += "<span>";
            secureUrl += "" + fileName + "&nbsp;&nbsp;";
            if (ext != ".docx" && ext != ".doc" && ext != ".xlsx" && ext != ".xls")
            {
                secureUrl += "<a target='_blank' href='/Reso?secureToken=" + EncodedPath + "&t=" + EncodePath(DateTime.UtcNow.ToString()) + "'>Open</a>&nbsp;";
            }
            secureUrl += "<a href='/Resd?secureToken=" + EncodedPath + "&t=" + EncodePath(DateTime.UtcNow.ToString()) + "'>Download</a>";
            secureUrl += "</span>";
        }
        return secureUrl;
    }
    public string GetPublicUrl(string fullPath, string fileName)
    {
        string EncodedPath = EncodePath(fullPath);
        string publicUrl = "";
        if (!string.IsNullOrEmpty(EncodedPath) && File.Exists(fullPath))
        {
            publicUrl = "<a href='/Resp?secureToken=" + EncodedPath + "'>" + fileName + "</a>&nbsp;";
        }
        return publicUrl;
    }
    public string GetPlainUrl(string fullPath)
    {
        string EncodedPath = EncodePath(fullPath);
        string publicUrl = "";
        if (!string.IsNullOrEmpty(EncodedPath) && File.Exists(fullPath))
        {
            publicUrl = "/Resl?secureToken=" + EncodedPath;
        }
        return publicUrl;
    }
    public string DisplaySecureImage(string fullPath)
    {
        string EncodedPath = EncodePath(fullPath);
        string secureUrl = "";
        if (!string.IsNullOrEmpty(EncodedPath) && File.Exists(fullPath))
        {
            secureUrl = "/Resv?secureToken=" + EncodedPath + "";
        }
        else
        {
            secureUrl = "https://advisor.aiu.edu/images/NoImages.jpg";
        }
        return secureUrl;
    }
    public bool ValidateUrlExpiryDate(DateTime sentDate)
    {
        var td = DateTime.UtcNow - sentDate;
        int minutes = Convert.ToInt32(td.TotalMinutes);
        if (minutes <= 10)
        {
            return true;
        }
